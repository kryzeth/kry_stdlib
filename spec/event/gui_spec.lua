...
describe('Gui', function()
    ...
    it('.on_checked_state_changed should register a wrapper handler with Event.register', function()
        local s = spy.on(Event, "register")
        Gui.on_checked_state_changed( "test_pattern", function_a )
        assert.spy(s).was_called()
        assert.spy(s).was_called_with(defines.events.on_gui_checked_state_changed, match.is_callable())
    end)

    it('.on_text_changed should register a wrapper handler with Event.register', function()
        local s = spy.on(Event, "register")
        Gui.on_text_changed( "test_pattern", function_a )
        assert.spy(s).was_called()
        assert.spy(s).was_called_with(defines.events.on_gui_text_changed, match.is_callable())
    end)

    it('.on_elem_changed should register a wrapper handler with Event.register', function()
        local s = spy.on(Event, "register")
        Gui.on_elem_changed( "test_pattern", function_a )
        assert.spy(s).was_called()
        assert.spy(s).was_called_with(defines.events.on_gui_elem_changed, match.is_callable())
    end)

    it('.on_selection_state_changed should register a wrapper handler with Event.register', function()
        local s = spy.on(Event, "register")
        Gui.on_selection_state_changed( "test_pattern", function_a )
        assert.spy(s).was_called()
        assert.spy(s).was_called_with(defines.events.on_gui_selection_state_changed, match.is_callable())
    end)
end)
